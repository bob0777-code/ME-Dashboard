...
function Storage.prepare()
 -- Data.update() in dashboard handles all ME loading now.
end
...
function Storage.draw(area)
 ...
 local p=percent(stats.used,stats.bytes)
 Renderer.write(rightX,area.y+6,"Cell Usage",Theme.muted)
 Renderer.write(rightX,area.y+7,Utils.bar(stats.used,stats.bytes,math.min(26,rightW)),Theme.good)
 Renderer.write(rightX,area.y+8,Utils.formatNumber(stats.used).." / "..Utils.formatNumber(stats.bytes).." Bytes ("..p.."%)",Theme.header)

 Renderer.write(rightX,area.y+10,"Storage Cells",Theme.muted)
 Renderer.write(rightX+18,area.y+10,tostring(stats.cells),Theme.good)

 Renderer.write(rightX,area.y+12,"Item Capacity",Theme.muted)
 Renderer.write(rightX+18,area.y+12,Utils.formatNumber(stats.itemCap).." Items",Theme.header)

 Renderer.write(rightX,area.y+14,"Fluid Capacity",Theme.muted)
 Renderer.write(rightX+18,area.y+14,Utils.formatNumber((stats.fluidUsed or 0)/1000).." / "..Utils.formatNumber((stats.fluidCap or 0)/1000).." Buckets",Theme.header)

 Renderer.write(rightX,area.y+16,"Stored Energy",Theme.muted)
 Renderer.write(rightX+18,area.y+16,Utils.formatNumber(stats.energy).." FE",Theme.header)
